...
object dominanceScore_4d_utils {
	 ...
	 */
	def calculate_dominance_score_4d(df1:DataFrame, df2:DataFrame, sparkSession: SparkSession, scoreToAdd:Int): DataFrame ={
	...
	ListBuffer[((Int, Int, Int, Int), (Int, Double, Double, Double, Double, Double, Double, Double, Double), (Int, Int))] ={
		...
		for(i <- 0 to x_axis_size){
			for(j <- 0 to y_axis_size){
				for(l <- 0 to z_axis_size) {
					for(h <- 0 to t_axis_size){
						val grid_cell = (i, j, l, h)

						val borders = get_cell_borders4d(
							x_max,
							y_max,
							z_max,
							t_max,
						...
						val t_line_high = borders(6)
						val t_line_low = borders(7)
		...
		var candidate_grid_cells = new ListBuffer[((Int, Int, Int, Int), (Int, Double, Double, Double, Double, Double, Double, Double, Double))]

		for(grid_cell <- grid_cells_with_counts_map){
			var number_of_dominating_points = 0

			for(dominating_cell <- ((grid_cell._1._1 -1 to 0) by -1 zip (grid_cell._1._2 - 1 to 0 by -1))
				zip ((grid_cell._1._3 -1 to 0) by -1 zip (grid_cell._1._4 - 1 to 0 by -1))  map {
				case ((x, y), (z, t))=> (x, y, z, t)}){

				if(dominating_cell._1 >= 0 && dominating_cell._2 >= 0 && dominating_cell._3 >= 0 && dominating_cell._4 >= 0 ) {
		...
		var candidate_grid_cells_with_bound_scores =
			new ListBuffer[((Int, Int, Int, Int), (Int, Double, Double, Double, Double, Double, Double, Double, Double), (Int, Int))]
		...
		for(grid_cell <- candidate_grid_cells){
			...
			var h = grid_cell._1._4
			for(i <- grid_cell._1._1 to x_axis_size){
				partially_dominated_points_count += grid_cells_with_counts_map((i, j, l, h))._1
			}
			...
			j = grid_cell._1._2
			l = grid_cell._1._3
			for(h <- grid_cell._1._4 to t_axis_size){
				partially_dominated_points_count += grid_cells_with_counts_map((i, j, l, h))._1
			}

			partially_dominated_points_count -= grid_cell._2._1 * 3
			...
			for(i <- grid_cell._1._1 to x_axis_size)
				for(j <- grid_cell._1._2 to y_axis_size)
					for(l <- grid_cell._1._3 to z_axis_size)
						for(h <- grid_cell._1._4 to t_axis_size)
							partially_and_fully_dominated_points_count += grid_cells_with_counts_map((i, j, l, h))._1
	 ...
	 */
	def get_cell_borders4d(x_max:Double,
												 y_max:Double,
												 z_max:Double,
												 t_max:Double,
												 grid_cell:(Int, Int, Int, Int),
												 x_axis:List[Double],
												 y_axis:List[Double],
												 z_axis:List[Double],
												 t_axis:List[Double]): List[Double] ={
		...
		var t_line_high = t_max
		if (grid_cell._4 != t_axis.size) {
			t_line_high =  t_axis(grid_cell._4)
		}
		var t_line_low = .0
		if (grid_cell._4 != 0){
			t_line_low = t_axis(grid_cell._4 - 1)
		}

		List(x_line_left, x_line_right, y_line_up, y_line_down, z_line_high, z_line_low, t_line_high, t_line_low)
	}

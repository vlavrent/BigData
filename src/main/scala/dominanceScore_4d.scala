...
object dominanceScore_4d {
	...
	def get_top_k_dominant_4d(k:Int, dataset_path:String,  x_axis_size:Int, y_axis_size:Int, z_axis_size:Int, t_axis_size:Int): Unit ={
		...
		val conf = new SparkConf()
			.setMaster("local[*]")
			.setAppName("DominanceScore2d_top" + k + "_" + dataset_path + "_" + x_axis_size + "_" + y_axis_size + "_" + z_axis_size + "_" + t_axis_size)
		...
		for(grid_cell <- grid_cells_to_check){
			...
			val cells_to_check_dominance_df = df.filter(
				"( x > " + x_line_right + " AND y <= " + y_line_up + " AND y > " + y_line_down +
					" AND z <= " + z_line_high + " AND z > " + z_line_low + " AND t <= " + t_line_high + " AND t > " + t_line_low + " ) " +
					" OR " + " ( y > " + y_line_up + " AND  x <= " + x_line_right +  " AND x > " + x_line_left +
					" AND z <= " + z_line_high + " AND z > " + z_line_low + " AND t <= " + t_line_high + " AND t > " + z_line_low + " ) " +
					" OR " + " ( z > " + z_line_high + " AND  x <= " + x_line_right +  " AND x > " + x_line_left +
					" AND y <= " + y_line_up + " AND y > " + y_line_down + " AND t <= " + t_line_high + " AND t > " + t_line_low +" ) " +
					" OR " + " ( t > " + t_line_high + " AND  x <= " + x_line_right +  " AND x > " + x_line_left +
					" AND y <= " + y_line_up + " AND y > " + y_line_down + " AND z <= " + z_line_high + " AND z > " + z_line_low + " ) " )
